...
def main():
    ...
    with st.sidebar:
        ...
        st.metric("Test F1-Score", "0.89")
        st.metric("Independent Dataset F1", "0.88")
        st.metric("ROC AUC", "0.89")
        st.metric("Sensitivity (TPR)", "0.82")
        st.metric("Specificity (TNR)", "0.87")
        st.metric("Decision Threshold", f"{OPTIMAL_THRESHOLD}")
        st.divider()
        st.write("**Model Performance:**")
        st.write("- Catches 89% of tumors")

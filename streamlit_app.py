...
def main():
    ...
    with st.sidebar:
        ...
        st.metric("Test F1-Score", "0.98")
        st.metric("Independent Dataset F1", "0.97")
        st.metric("ROC AUC", "0.99")
        st.metric("Sensitivity (TPR)", "0.98")
        st.metric("Specificity (TNR)", "0.99")
        st.metric("Decision Threshold", f"{OPTIMAL_THRESHOLD}")
        st.divider()
        st.write("**Model Performance:**")
        st.write("- Catches 98% of tumors")
